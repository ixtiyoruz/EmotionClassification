...
def get_model(config, backbone_config_path=None, model_checkpoint_path=None, train=True):
    ...
    if train:
        ...
        if config.model.freeze_n_layers > 0:
            freeze(model.backbone.encoder.layer[:config.freeze_n_layers])
        if config.model.reinitialize_n_layers > 0:
            layers = getattr(model.backbone.encoder, 'layer', None)
            if(layers is None):
                layers = getattr(model.backbone.encoder, 'layers', None)
            for module in layers[-config.model.reinitialize_n_layers:]:
